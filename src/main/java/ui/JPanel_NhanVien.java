...
public class JPanel_NhanVien extends JPanel implements ActionListener, MouseListener {
	 ...
	 */
	public JPanel_NhanVien() throws RemoteException {
		try {

			ChucVu_DAO = ClientDAO.getChucVu_DAO();
			NhanVien_DAO = ClientDAO.getNhanVien_DAO();
		} catch (Exception e) {
		...
		panel.setBorder(
				new TitledBorder(new EtchedBorder(EtchedBorder.LOWERED, new Color(0, 85, 170), new Color(0, 85, 170)),
						"Qua\u0309n ly\u0301 nh\u00E2n vi\u00EAn", TitledBorder.LEADING, TitledBorder.TOP, null,
						new Color(0, 85, 170)));
		...
		ArrayList<entities.ChucVu> listCV = ChucVu_DAO.getAllChucVu();
		for (entities.ChucVu cv : listCV) {
		...
		panel_2.setBorder(
				new TitledBorder(new EtchedBorder(EtchedBorder.LOWERED, new Color(0, 85, 170), new Color(0, 85, 170)),
						"Danh sa\u0301ch nh\u00E2n vi\u00EAn", TitledBorder.LEADING, TitledBorder.TOP, null,
						new Color(0, 85, 170)));
	...
	private void DocNhanVienTuDB() throws RemoteException {
		ArrayList<entities.NhanVien> dsNV = ClientDAO.getNhanVien_DAO().getAllTableNhanVien();
		for (entities.NhanVien nv : dsNV) {
			Object[] obj = new Object[] { nv.getMaNhanVien(), nv.getTenNhanVien(), nv.getDiaChi(), nv.getSdt(),
					nv.getEmail(), nv.getChucVu().getTenChucVu(), nv.getNgaySinh(), nv.isGioiTinh() ? "nữ" : "nam" };
	...
	private void capNhatModelNhanVien() throws RemoteException {
	...
	private void themNhanVienVaoModel(NhanVien nv) {
		Object[] obj = new Object[] { nv.getMaNhanVien(), nv.getTenNhanVien(), nv.getDiaChi(), nv.getSdt(),
				nv.getEmail(), nv.getChucVu().getTenChucVu(), nv.getNgaySinh(), nv.isGioiTinh() ? "Nam" : "Nữ" };
		modelQuanLyNhanVien.addRow(obj);
	}

	// Tạo nhân viên từ field:
	private NhanVien taoNhanVienTuFeild() throws RemoteException {
	...
	public void xoaNV() throws RemoteException {
		int row = tableQuanLyNhanVien.getSelectedRow();
		String maNV = modelQuanLyNhanVien.getValueAt(row, 0).toString();
		int choice = JOptionPane.showConfirmDialog(null, "Bạn có chắc chắn muốn xoá?", "xác nhận xoá!",
	...
	private boolean isValidNhanVien() {
		...
		if (!(tenNV.length() > 0 && tenNV.matches("^[\\p{Lu}][\\p{L}]+([\\s][\\p{Lu}][\\p{L}]+)*$"))) {
			JOptionPane.showMessageDialog(this,
					"Tên nhân viên không hợp lệ. Hãy viết hoa chữ cái đầu và có thể có dấu.");
...
//		}
	...
	public void themNV() throws RemoteException {
		if (isValidNhanVien()) {
	...
	public void suaNV() throws RemoteException {
		if (isValidNhanVien()) {
			NhanVien nv = taoNhanVienTuFeild();
