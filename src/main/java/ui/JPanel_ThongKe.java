...
public class JPanel_ThongKe extends JPanel {
	 ...
	 */
	public JPanel_ThongKe() throws RemoteException {
		try {

///////
			dao = ClientDAO.getThongKe_DAO();
			hoaDon_dao = ClientDAO.getHoaDon_DAO();
		} catch (Exception e) {
...
//////
	 ...
	 * @throws RemoteException
	 */
	public void docDuLieuVaoTableLinhKien() throws RemoteException {
	...
	public void docDuLieuVaoTableKhachHang() throws RemoteException {
		...
		for (HoaDon hd : khachhang) {
			...
			KhachHang makh = hd.getKhachHang();
			...
			for (ChiTietHoaDon cthd : chiTietHoaDonList) {
				...
				if (thongTinKhachHang.containsKey(makh.getMaKhachHang())) {
					...
					thongTinKhachHang.put(makh.getMaKhachHang(),
							new Object[] { tongSoLuong, tongThanhTien, tekhachhang });
				} else {
	...
	public void docDuLieuVaoTableNhanVien() throws RemoteException {
	...
	public void tinhDoanhThu() throws RemoteException {
	...
	/**
	 * tính doanh thu và hiển thị table sản phẩm , nhân viên , khách hàng
	 * trong hôm nay
	 * 
	 * @throws RemoteException
	 */
	public void hienThiSanPhamBanChayVaDoanhThuHomNay() throws RemoteException {
	...
	public void hienThiKhachHangvaDoanhThuHomNay() throws RemoteException {
		// Lấy danh sách hóa đơn trong ngày hôm nay
		ArrayList<HoaDon> doanhthu = dao.layDsHoaDon();
		DecimalFormat df = new DecimalFormat("###,###");
		...
		for (int row = 0; row < modelTopKhachHangTongTienCao.getRowCount(); row++) {
			double thanhTien = Double
					.parseDouble(modelTopKhachHangTongTienCao.getValueAt(row, 3).toString().replace(",", ""));
			tongDoanhThuTuTableModel += thanhTien;
		}
		lblDoanhThu.setText(df.format(tongDoanhThuTuTableModel));
	}

	public void hienThiNhanVienvaDoanhThuHomNay() throws RemoteException {
		// Lấy danh sách hóa đơn trong ngày hôm nay
		ArrayList<HoaDon> doanhthu = dao.layDsHoaDon();
		DecimalFormat df = new DecimalFormat("###,###");
		double tongDoanhThuHomNay = 0;
		...
		for (HoaDon hd : doanhthu) {
			...
			if (ngayLap.equals(homNay)) {
				...
				if (thongTinNhanVien.containsKey(manv.getMaNhanVien())) {
					...
					// Nếu chưa có, thêm mới thông tin vào Map
					thongTinNhanVien.put(manv.getMaNhanVien(),
							new Object[] { chiTietHoaDonList.size(), tongDoanhThuHoaDon, tennv });
				}
	 ...
	 * @throws RemoteException
	 */
	public void hienThiSanPhamBanChayTheoThang() throws RemoteException {
		ArrayList<HoaDon> doanhthu = dao.layDsHoaDon();
		DecimalFormat df = new DecimalFormat("###,###");
		double tong = 0;
	...
	/**
	 * tinh doanh thu theo thang va hien thi table khach hang co tong tien cao nhat
	 * trong thang
	 * 
	 * @throws RemoteException
	 */
	public void hienThiDoanhThuTheoThangChoKhachHang() throws RemoteException {
		ArrayList<HoaDon> doanhthu = dao.layDsHoaDon();
		DecimalFormat df = new DecimalFormat("###,###");
		double tong = 0;
		int soLuongHoaDonTrongThang = 0;
		...
		for (HoaDon hd : doanhthu) {
			...
			if (ngayLap.getMonthValue() == thangChon && ngayLap.getYear() == namHienTai) {
				...
				if (thongTinKhachHang.containsKey(makh.getMaKhachHang())) {
					...
					thongTinKhachHang.put(makh.getMaKhachHang(), new Object[] { 1, tongHoaDon, tekhachhang }); // Start
																												// with
																												// 1 for
																												// a new
																												// customer
	...
	/**
	 * tinh doanh thu theo thang va hien thi trong table nhan vien co doanh thu cao
	 * nhat trong thang
	 * 
	 * @throws RemoteException
	 * 
	 */
	public void hienThiDoanhThuTheoThangCuaNhanVien() throws RemoteException {
		ArrayList<HoaDon> doanhthu = dao.layDsHoaDon();
		DecimalFormat df = new DecimalFormat("###,###");
		int soLuongHoaDonTrongThang = 0;
		double tong = 0;
		...
		for (HoaDon hd : doanhthu) {
			...
			if (ngayLap.getMonthValue() == thangChon && ngayLap.getYear() == namHienTai) {
				...
				if (thongTinNhanVien.containsKey(manv.getMaNhanVien())) {
					...
					thongTinNhanVien.put(manv.getMaNhanVien(),
							new Object[] { chiTietHoaDonList.size(), tongHoaDon, tennv });
				}

				// Increase the count when encountering an invoice in the selected month and
				// year
		...
		for (Map.Entry<String, Object[]> entry : sortedList) {
			...
			modelTopNhanVienCoDoanhThuCaoNhat
					.addRow(new Object[] { i++, maNhanVien, tennv.getTenNhanVien(), df.format(tongThanhTien) });
		}
	}
	 ...
	 * @throws RemoteException
	 */
	public void hienThiTopSanPhamVaDoanhThuTheoNam() throws RemoteException {
		ArrayList<HoaDon> doanhthu = dao.layDsHoaDon();
		DecimalFormat df = new DecimalFormat("###,###");
	...
	public void hienThiThongTinKhachHangVaDoanhThuTheoNam() throws RemoteException {
		ArrayList<HoaDon> doanhthu = dao.layDsHoaDon();
		DecimalFormat df = new DecimalFormat("###,###");
		double tongDoanhThuTrongNam = 0;
		int soLuongHoaDonTrongNam = 0;
		...
		// Sử dụng List để lưu thông tin cho mỗi khách hàng và sắp xếp giảm dần theo
		// tổng tiền
		...
		for (HoaDon hd : doanhthu) {
			...
			if (ngayLap.getYear() == namChon) {
				...
				KhachHang teKhachHang = dao.timKiemTheoMaKhachHang(hd.getKhachHang());
				...
				if (!found) {
					...
					thongTinKhachHangList
							.add(new Object[] { maKhachHang, chiTietHoaDonList.size(), tongHoaDon, teKhachHang });
				}
			}
		}
	...
	public void hienThiThongTinNhanVienVaDoanhThuTheoNam() throws RemoteException {
		ArrayList<HoaDon> doanhthu = dao.layDsHoaDon();
		DecimalFormat df = new DecimalFormat("###,###");
		double tongDoanhThuTrongNam = 0;
		int soLuongHoaDonTrongNam = 0;
		...
		for (HoaDon hd : doanhthu) {
			...
			if (ngayLap.getYear() == namChon) {
				...
				NhanVien teNhanVien = dao.searchMaNhanVien(hd.getNhanVien());
		...
		int i = 1;
		DefaultTableModel modelTopNhanVienCoDoanhThuCaoNhat = (DefaultTableModel) tableTopNhanVienCoDoanhThuCaoNhat
				.getModel();
		modelTopNhanVienCoDoanhThuCaoNhat.setRowCount(0);
		List<Map.Entry<String, Object[]>> sortedList = new ArrayList<>(thongTinNhanVien.entrySet());
		...
		for (Map.Entry<String, Object[]> entry : sortedList) {
			...
			modelTopNhanVienCoDoanhThuCaoNhat
					.addRow(new Object[] { i++, maNhanVien, teNhanVien.getTenNhanVien(), df.format(tongThanhTien) });
		}
	}
