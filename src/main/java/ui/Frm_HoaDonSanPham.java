...
public class Frm_HoaDonSanPham extends JFrame {
	...
	public Frm_HoaDonSanPham() throws SQLException, RemoteException {
		JScrollPane sc = new JScrollPane();
		setDefaultCloseOperation(DISPOSE_ON_CLOSE);
		setSize(new Dimension(425, 600));
		String maNV = Frm_TrangChu.txtMaNV.getText();
//////
		NhanVien_DAO nv_dao = ClientDAO.getNhanVien_DAO();
...
///////	
		...
		for (int i = 0; i < tableRowCount; i++) {
			...
			double thanhTien = Double
					.parseDouble(JPanel_HoaDon.tableChiTietHoaDon.getValueAt(i, 4).toString().replace(",", ""));
			DecimalFormat df = new DecimalFormat("#,###,###,###  VNĐ");
			String thanhTien1 = df.format(thanhTien);
			tongtien += thanhTien;
			tenSanPham = (tenSanPham.length() > 64)
					? tenSanPham.substring(0, 64) + "\n" + "  " + tenSanPham.substring(64, tenSanPham.length())
					: tenSanPham;
